...
from .exceptions import InitializationError, InterfaceNotImplementedError
from .interfaces import VALID_INTERFACES, LIN_VALID_INTERFACES

log = logging.getLogger("canlin.util")
...
CAN_CONFIG_FILES = ["~/can.conf"]
LIN_CONFIG_FILES = ["~/lin.conf"]

if platform.system() == "Linux":
    CAN_CONFIG_FILES.extend(["/etc/can.conf", "~/.can", "~/.canrc"])
    LIN_CONFIG_FILES.extend(["/etc/lin.conf", "~/.lin", "~/.linrc"])
elif platform.system() == "Windows" or platform.python_implementation() == "IronPython":
    CAN_CONFIG_FILES.extend(["can.ini", os.path.join(os.getenv("APPDATA", ""), "can.ini")])
    LIN_CONFIG_FILES.extend(["lin.ini", os.path.join(os.getenv("APPDATA", ""), "lin.ini")])


def load_file_config(
    type_: Optional[str] = "CAN",
    ...
    if path is None:
        if type_ == "CAN":
            files = CAN_CONFIG_FILES
        else:
            files = LIN_CONFIG_FILES
        config.read([os.path.expanduser(path) for path in files])
    else:
        config.read(path)
...
def load_environment_config(type_: Optional[str] = "CAN", context: Optional[str] = None) -> Dict[str, str]:
    """
    ...
    * CAN_CONFIG_ABC
    ...
    """
    mapper = {
        "interface": f"{type_}_INTERFACE",
        "channel": f"{type_}_CHANNEL",
        "bitrate": f"{type_}_BITRATE",
    }

    context_suffix = f"_{context}" if context else ""
    can_config_key = f"{type_}_CONFIG{context_suffix}"
...
def load_config(
    ...
    :raises:
        InterfaceNotImplementedError if the ``interface`` name isn't recognized
    """
    ...
    config_sources = cast(
        ...
        [
            ...
            lambda _context: load_environment_config(type_),
            lambda _context: load_file_config(type_, path, _context),
            lambda _context: load_file_config(type_, path),
        ],
    )
    ...
    bus_config = _create_bus_config(type_, config)
    log.debug(f"{type_} config: %s", bus_config)
    return bus_config


def _create_bus_config(type_: str, config: Dict[str, Any]) -> typechecking.BusConfig:
    """Validates some config values, performs compatibility mappings and creates specific
    structures (e.g. for bit timings).

    :param type_: Bus type, ex. "CAN" or "LIN".
    :param config: The raw config as specified by the user
    :return: A config that can be used by a :class:`~BusABC`
    ...
    if type_ == "CAN":
        ...
        interfaces = LIN_VALID_INTERFACES
...
def check_or_adjust_timing_clock(timing: T, valid_clocks: Iterable[int]) -> T:
    ...
    :param timing:
        The :class:`~BitTiming` or :class:`~BitTimingFd` instance to adjust.
    :param valid_clocks:
    ...
    :return:
        A new :class:`~BitTiming` or :class:`~BitTimingFd` instance with an
...
if __name__ == "__main__":
    all_files = {"CAN": CAN_CONFIG_FILES, "LIN": LIN_CONFIG_FILES}
    for _type, _files in all_files.items():
        print(f"Searching for {_type} configuration named:")
        print("\n".join(_files))
        print()
        print("Settings:")
        print(load_config(_type))
