...
log = logging.getLogger("canlin.bcm")
...
class CyclicSendTaskABC(CyclicTask):
    ...
    def __init__(
        self,
        messages: Union[Sequence[Message], Message, Sequence[LinMessage], LinMessage],
        period: float
    ) -> None:
        """
        :param messages:
    ...
    def _check_and_convert_messages(
        messages: Union[Sequence[Message], Message, Sequence[LinMessage], LinMessage]
    ) -> Tuple[Union[Message, LinMessage], ...]:
        ...
        if not isinstance(messages, (list, tuple)):
            if isinstance(messages, Message) or isinstance(messages, LinMessage):
                messages = [messages]
            else:
        ...
        all_same_id = all(
            message.id == messages[0].id for message in messages
        )
        if not all_same_id:
...
class ModifiableCyclicTaskABC(CyclicSendTaskABC):
    ...
    def _check_modified_messages(self, messages: Tuple[Union[Message, LinMessage], ...]) -> None:
    ...
    def modify_data(self, messages: Union[Sequence[Message], Message, Sequence[LinMessage], LinMessage]) -> None:
...
class ThreadBasedCyclicSendTask(
    ...
    def start(self) -> None:
        ...
        if self.thread is None or not self.thread.is_alive():
            name = f"Cyclic send task for 0x{self.messages[0].id:X}"
