"""
This module contains the implementation of `Listener` and some readers.
"""
...
class Listener(metaclass=ABCMeta):
    ...
    @abstractmethod
    def on_message_received(self, msg: Union[Message, LinMessage]) -> None:
        """This method is called to handle the given message.

        :param msg: the delivered message
        """

    def __call__(self, msg: Union[Message, LinMessage]) -> None:
        self.on_message_received(msg)

    def on_error(self, exc: Exception) -> None:
        """This method is called to handle any exception in the reception thread.
...
class BufferedReader(Listener):  # pylint: disable=abstract-method
    """
    A BufferedReader is a subclass of :class:`~Listener` which implements a
    **message buffer**: that is, when the :class:`BufferedReader` instance is
    ...
    :meth:`~BufferedReader.get_message`.

    Putting in messages after :meth:`~BufferedReader.stop` has been called will raise
    an exception, see :meth:`~BufferedReader.on_message_received`.
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.buffer: SimpleQueue[Union[Message, LinMessage]] = SimpleQueue()
    ...
    def get_message(self, timeout: float = 0.5) -> Optional[Union[Message, LinMessage]]:
        """
        ...
        not block after :meth:`BufferedReader.stop` has been called.

        :param timeout: The number of seconds to wait for a new message.
        :return: the received :class:`Message`, :class:`LinMessage` or `None`, if the queue is empty.
        """
        try:
            if self.is_stopped:
...
class AsyncBufferedReader(
    Listener, AsyncIterator[Union[Message, LinMessage]]
):  # pylint: disable=abstract-method
    """A message buffer for use with :mod:`asyncio`.

    See :ref:`asyncio` for how to use with :class:`Notifier`.
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.buffer: "asyncio.Queue[Union[Message, LinMessage]]"
    ...
    async def get_message(self) -> Union[Message, LinMessage]:
        """
    ...
    def __aiter__(self) -> AsyncIterator[Union[Message, LinMessage]]:
        return self

    async def __anext__(self) -> Union[Message, LinMessage]:
        return await self.buffer.get()
