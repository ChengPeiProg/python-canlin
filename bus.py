...
from . import broadcastmanager, typechecking
from .broadcastmanager import CyclicSendTaskABC, ThreadBasedCyclicSendTask
from .message import Message, LinMessage

LOG = logging.getLogger(__name__)


class BusState(Enum):
    """The state in which a :class:`canlin.BusABC` can be."""
...
class BusABC(metaclass=ABCMeta):
    ...
    def __init__(
        self,
        channel: Any,
        filters: Optional[Union[typechecking.CanFilters, typechecking.Filter]] = None,
        **kwargs: object,
    ):
        ...
        :param filters:
            See :meth:`~BusABC.set_filters` for details.

        :param dict kwargs:
            Any backend dependent configurations are passed in this dictionary

        :raises ValueError: If parameters are out of range
        :raises ~exceptions.InterfaceNotImplementedError:
            If the driver cannot be accessed
        :raises ~exceptions.InitializationError:
    ...
    def recv(self, timeout: Optional[float] = None) -> Optional[Union[Message, LinMessage]]:
        ...
        :return:
            :obj:`None` on timeout or a :class:`~Message` object.

        :raises ~exceptions.CanOperationError:
    ...
    def _recv_internal(
    ...
    ) -> Tuple[Optional[Union[Message, LinMessage]], bool]:
        """
        Read a message from the bus and tell whether it was filtered.
        This methods may be called by :meth:`~BusABC.recv`
        to read a message multiple times if the filters set by
        :meth:`~BusABC.set_filters` do not match and the call has
        not yet timed out.

        New implementations should always override this method instead of
        :meth:`~BusABC.recv`, to be able to take advantage of the
        software based filtering provided by :meth:`~BusABC.recv`
        ...
        .. note::
            ...
            :meth:`~BusABC.recv` implementation.
        ...
        :raises NotImplementedError:
            if the bus provides it's own :meth:`~BusABC.recv`
    ...
    @abstractmethod
    def send(self, msg: Union[Message, LinMessage], timeout: Optional[float] = None) -> None:
    ...
    def send_periodic(
        self,
        msgs: Union[Message, Sequence[Message], LinMessage, Sequence[LinMessage]],
        period: float,
        ...
        - the task's :meth:`~broadcastmanager.CyclicTask.stop` method is called.
        ...
        :return:
            ...
            :meth:`~broadcastmanager.CyclicTask.stop` method.
        ...
        """
        if isinstance(msgs, Message) or isinstance(msgs, LinMessage):
            msgs = [msgs]
    ...
    def __iter__(self) -> Iterator[Union[Message, LinMessage]]:
    ...
    @property
    def filters(self) -> Optional[Union[typechecking.CanFilters, typechecking.Filter]]:
        """
        Modify the filters of this bus. See :meth:`~BusABC.set_filters`
        for details.
        """
        return self._filters

    @filters.setter
    def filters(self, filters: Optional[Union[typechecking.CanFilters, typechecking.Filter]]) -> None:
        self.set_filters(filters)

    def set_filters(
        self, filters: Optional[Union[typechecking.CanFilters, typechecking.Filter]] = None
    ) -> None:
    ...
    def _apply_filters(self, filters: Optional[Union[typechecking.CanFilters, typechecking.Filter]]) -> None:
        """
    ...
    def _matches_filters(self, msg: Union[Message, LinMessage]) -> bool:
        """Checks whether the given message matches at least one of the
        current filters. See :meth:`~BusABC.set_filters` for details
        ...
        for _filter in self._filters:
            ...
            if isinstance(msg, Message) and "extended" in _filter:
            ...
            # `msg.id & can_mask == can_id & can_mask`
            # by using the shorter, but equivalent from below:
            if (can_id ^ msg.id) & can_mask == 0:
                return True
