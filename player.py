"""
Replays CAN traffic saved with logger back
to a CAN bus.
...
from .io.player import LogReader, MessageSync
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--file_name",
        dest="log_file",
        help="Path and base log filename, for supported types see LogReader.",
        default=None,
    )
    ...
    parser.add_argument(
        ...
        metavar="input-file",
        type=str,
        help="The file to replay. For supported types see LogReader.",
    )
    ...
    with _create_bus(results, **additional_config) as bus:
        with LogReader(results.infile, **additional_config) as reader:
            in_sync = MessageSync(
                cast(Iterable[Union[Message, LinMessage]], reader),
            ...
            try:
                for message in in_sync:
                    # todo: class LinMessage has no attribute "is_error_frame"
