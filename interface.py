"""
This module contains the base implementation of :class:`canlin.BusABC`, :class:`canlin.LinBusABC` as well
...
log = logging.getLogger("canlin.interface")
...
class Bus(BusABC):  # pylint: disable=abstract-method
    ...
    :raises ~canlin.exceptions.InterfaceNotImplementedError:
        if the ``interface`` isn't recognized or cannot be loaded

    :raises ~canlin.exceptions.InitializationError:
...
def detect_available_configs(
    ...
    :return: an iterable of dicts, each suitable for usage in
             the constructor of :class:`BusABC`.
    """
