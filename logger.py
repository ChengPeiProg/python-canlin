...
import can
from .io import BaseRotatingLogger
from .io.generic import MessageWriter
from .util import cast_from_string

from . import Bus, BusState, Logger, SizedRotatingLogger
from .typechecking import Filter, CanFilters
    ...
    **kwargs: Any,
    ...
    parser.add_argument(
        *args,
        ...
        "\n      python -m viewer -f 100:7FC 200:7F0"
...
def _create_bus(parsed_args: Any, **kwargs: Any) -> Bus:
    logging_level_names = ["critical", "error", "warning", "info", "debug", "subdebug"]
    set_logging_level(logging_level_names[min(5, parsed_args.verbosity)])
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--file_name",
        dest="log_file",
        help="Path and base log filename, for supported types see Logger.",
        default=None,
    )
...
# todo: to add lin support into cmd
