...
class ThreadSafeBus(ObjectProxy):  # pylint: disable=abstract-method
    """
    Contains a thread safe :class:`BusABC` implementation that
    ...
    Use this as a drop-in replacement for :class:`~BusABC`.

    .. note::

        This approach assumes that both :meth:`~BusABC.send` and
        :meth:`~BusABC._recv_internal` of the underlying bus instance can be
        called simultaneously, and that the methods use :meth:`~BusABC._recv_internal`
        instead of :meth:`~BusABC.recv` directly.
    """
