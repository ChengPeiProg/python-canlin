...
class ComError(Exception):
    ...
    .. testsetup:: canerror
        ...
        from can import ComError, CanOperationError

    .. doctest:: canerror

        >>> # With an error code (it also works with a specific error):
        >>> error = OperationError(message="Failed to do the thing", error_code=42)
        >>> str(error)
        'Failed to do the thing [Error Code 42]'
        >>>
        >>> # Missing the error code:
        >>> plain_error = ComError(message="Something went wrong ...")
        >>> str(plain_error)
...
class InterfaceNotImplementedError(ComError, NotImplementedError):
...
class InitializationError(ComError):
    """Indicates an error the occurred while initializing a :class:`BusABC`.

    If initialization fails due to a driver or platform missing/being unsupported,
    a :exc:`~exceptions.InterfaceNotImplementedError` is raised instead.
...
class TimeoutError(ComError, TimeoutError):
...
def error_check(
    ...
    exception_type: Type[ComError] = OperationError,
