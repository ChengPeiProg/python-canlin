"""
This module contains the implementation of :class:`~Notifier`.
"""
...
logger = logging.getLogger("canlin.Notifier")

MessageRecipient = Union[Listener, Callable[[Union[Message, LinMessage]], Union[Awaitable[None], None]]]
...
class Notifier:
    def __init__(
        ...
        """Manages the distribution of :class:`~Message` instances to listeners.
        ...
        :param listeners:
            An iterable of :class:`~Listener` or callables that receive a :class:`~Message`
            and return nothing.
        :param timeout: An optional maximum number of seconds to wait for any :class:`~Message`.
    ...
    def add_bus(self, bus: BusABC) -> None:
        ...
        if self._loop is not None and reader >= 0:
            ...
            reader_thread = threading.Thread(
                ...
                args=(bus,),
                name=f'notifier for bus "{bus.channel_info}"',
            )
    ...
    def stop(self, timeout: float = 5) -> None:
        """Stop notifying Listeners when new :class:`~Message` objects arrive
        and call :meth:`~Listener.stop` on each Listener.
    ...
    def _on_message_received(self, msg: Union[Message, LinMessage]) -> None:
